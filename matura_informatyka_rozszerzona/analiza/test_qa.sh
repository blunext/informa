...
run_layer_1() {
  ...
  test_cmd "test-report summary (md)" "$MATURA" test-report summary --historia "$SCRIPT_DIR/test_pedagogical/reports/historia.json"
  test_json_cmd "test-report summary (json)" "$MATURA" test-report summary --historia "$SCRIPT_DIR/test_pedagogical/reports/historia.json" --format json
...
" 2>/dev/null) || max_pts="3"
  ...
  sqlite3 "$TMPDIR_QA/matura_progress.db" \
    "INSERT OR REPLACE INTO progress_meta (key, value) VALUES ('session_context_weight', '76')"
  weight_out=$(matura_tmp exercise next --typ sql_group_by 2>&1)
