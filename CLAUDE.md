...
│   ├── matura_YYYYS.json          # **12 files — COMPLETE exam database** (250 subtasks, self-contained)
│   │                              # Full task text, answers, scoring criteria, typ_zadania, traps
│   │                              # No PDF needed — each subtask is standalone and solvable
│   ├── matura_indeks.json         # **Cross-reference index** for all 250 subtasks across 12 exams
...
│   ├── matura.db                 # SQLite DB: 937 exercises + 250 CKE + 4 cheatsheets
...
- **Complete exam database**: `matura_YYYYS.json` — 12 files (11 maj + 1 czerwiec), 250 subtasks, 600 points total. Each subtask has full text (`tresc`), answer (`odpowiedz`), scoring (`zasady_oceniania`), type (`typ_zadania`), and traps (`pulapki`). Self-contained: no PDF needed to solve any task. ID format: `YYYYS.Z.S` (e.g. `2025M.1.1`, `2024C.3.2`), where S=session letter (M=maj, C=czerwiec).
- **Cross-reference index**: `matura_indeks.json` — all 250 subtasks indexed by typ_zadania, kategoria, rok. Supports filtering for cross-year practice (e.g., "all SQL tasks" or "all 2025 tasks").
